-- Fix the process_withdrawal function to prevent negative on_hold balances
...
BEGIN
  ...
  IF NEW.status = 'approved' AND OLD.status = 'pending' THEN
    ...
    -- Get current balance to check on_hold amount
    SELECT balance, on_hold INTO v_current_balance
    FROM public.user_balances
    WHERE user_id = NEW.user_id;
    
    -- Only subtract from on_hold if we have sufficient amount (prevent negative)
    IF v_current_balance.on_hold >= NEW.amount THEN
      ...
      WHERE user_id = NEW.user_id;
    ELSE
      -- If on_hold is insufficient, set it to 0 and log the discrepancy
      UPDATE public.user_balances
  ...
  ELSIF (NEW.status = 'rejected' OR NEW.status = 'failed') AND OLD.status = 'pending' AND NEW.processed_at IS NULL THEN
    ...
    IF v_current_balance.on_hold >= NEW.amount THEN
      ...
      END IF;
    ELSE
      -- If on_hold is insufficient, just set it to 0
      UPDATE public.user_balances
      SET on_hold = GREATEST(0, v_current_balance.on_hold), updated_at = now()
      WHERE user_id = NEW.user_id;
    END IF;
  END IF;
...
-- Fix the negative on_hold balance for the current user
UPDATE public.user_balances 
SET on_hold = 0, updated_at = now()
WHERE user_id = '18bc1315-c35d-4c35-8c11-ae287e44d1d2' AND on_hold < 0;
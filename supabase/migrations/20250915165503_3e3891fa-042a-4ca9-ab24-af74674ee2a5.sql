-- First, remove duplicate closing orders that already exist
WITH duplicates AS (
  SELECT id,
    ROW_NUMBER() OVER (
      PARTITION BY original_trade_id, user_id, symbol, side, entry_price, realized_pnl 
      ORDER BY created_at
    ) as rn
...
UNIQUE (original_trade_id, user_id, symbol, side);

-- Update the finalize_trade_positions function to be idempotent

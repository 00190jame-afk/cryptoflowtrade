...
AS $$
  ...
  JOIN public.admin_profiles ap ON ap.user_id = admin_user_id
...
USING (
  is_super_admin() OR 
  (is_any_admin() AND auth.uid() IN (
    SELECT admin_user_id FROM get_admin_users(auth.uid())
  ))
);
...
USING (
  ...
  (is_any_admin() AND user_id IN (
    SELECT user_id FROM get_admin_users(auth.uid())
  ))
);
